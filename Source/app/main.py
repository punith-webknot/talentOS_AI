...
from Source.app.tools.agent_tools import get_job_agent_tool, get_slots_agent_tool
from Source.app.tools.mcp_client import create_mcp_client, open_mcp_session
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Keep the MCP HTTP session open for the entire app lifetime so tools
        # stay bound to a live session (avoids per-call POST/DELETE churn).
        async with open_mcp_session(mcp_client) as (_mcp_session, mcp_tools):
            ...
            async with AsyncPostgresSaver.from_conn_string(settings.database_uri) as checkpointer:
                ...
                logger.info(
                    "job_agent tools=%s | slots_agent tools=%s | supervisor tools=%s",
